...
** TODO there's a filterable list on the left
*** TODO show the born filter toggle
*** DONE implement simple/dumb search on the backend
...
*** TODO need a JS pickup event like start? or just go by the type in elm
** TODO drop beacons should be distinct from draggable beacons
ie the closestBounding fn should filter on type

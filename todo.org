...
** currently storing lifepath/setting name, same as config file
** should these be storing lifepath/setting ids?
** they're not for the database, but for executing against an in-memory character sheet
** the names still need to stay in sync
*** should at least unit test the book lifepaths' requirements
